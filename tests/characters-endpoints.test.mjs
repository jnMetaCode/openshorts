...
// 假改图供应商：把 agnes 的 base_url 指到本机，记下收到的请求体（multipart 原文），回一张 JPEG
const http = await import('node:http');
const editSeen = [];
const JPG = Buffer.from([0xff, 0xd8, 0xff, 0xe0, 0, 4, 0, 0]);
const fakeProvider = await new Promise((ok) => { const s = http.createServer((q, r) => { const bufs = []; q.on('data', (d) => bufs.push(d)); q.on('end', () => {
  editSeen.push({ url: q.url, auth: q.headers.authorization, body: Buffer.concat(bufs).toString('latin1') });
  if (editSeen.length <= 2) { r.writeHead(503, { 'content-type': 'application/json' }); return r.end(JSON.stringify({ error: { message: 'no available server' } })); }
  r.writeHead(200, { 'content-type': 'application/json' }); r.end(JSON.stringify({ data: [{ b64_json: JPG.toString('base64') }] })); }); }); s.listen(0, '127.0.0.1', () => ok(s)); });
process.env.AGNES_BASE_URL = `http://127.0.0.1:${fakeProvider.address().port}/v1`;
process.env.AGNES_API_KEY = 'agnes-test-key';
...
test.after(() => { server.close(); fakeProvider.close(); fs.rmSync(home, { recursive: true, force: true }); });
...
test('场景卡接口 + 保脸改图：供应商报错原话透给界面；成功后存新图、记住这家；放进场景时两张图都发出去', async () => {
  const bad = await j(await post('/scenes', { name: '天台' }));
  assert.equal(bad.status, 400);
  const sc = await j(await post('/scenes', { name: '旧天台', place: '老居民楼天台，水箱，晾衣绳' }));
  assert.equal(sc.status, 200); assert.equal(sc.body.imageUrl, null);
  const png = Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a, 0, 0, 0, 13, 0x49, 0x48, 0x44, 0x52, 0, 0, 4, 0, 0, 0, 2, 0x40]);
  const up = await j(await fetch(`${base}/scenes/${encodeURIComponent(sc.body.id)}/upload`, { method: 'POST', headers: { 'content-type': 'image/png' }, body: png }));
  assert.equal(up.status, 200);
  assert.equal((await fetch(`http://127.0.0.1:${server.address().port}${up.body.imageUrl}`)).status, 200);
  const id = encodeURIComponent('阿澜');
  assert.equal((await j(await post(`/characters/${id}/edit`, { instruction: 'x' }))).status, 400, '没选供应商 / 模型');
  // 第一次供应商回 503：原话要到界面
  const e1 = await j(await post(`/characters/${id}/edit`, { provider: 'agnes', model: 'agnes-image-2.5-flash', instruction: '换成白衬衫' }));
  assert.equal(e1.status, 502); assert.match(e1.body.error, /no available server/);
  // 第二次成功，带场景
  const e2 = await j(await post(`/characters/${id}/edit`, { provider: 'agnes', model: 'agnes-image-2.5-flash', instruction: '', scene: sc.body.id }));
  assert.equal(e2.status, 200, JSON.stringify(e2.body));
  assert.equal(e2.body.portrait.source, 'cloud-edit'); assert.deepEqual(e2.body.portrait.scene, { id: sc.body.id, file: up.body.image.file });
  // 第一次：generations 与 edits 两种接口都 503，两边原话都在报错里
  assert.deepEqual(editSeen.slice(0, 2).map((x) => x.url), ['/v1/images/generations', '/v1/images/edits']);
  const last = editSeen.at(-1);
  assert.equal(last.url, '/v1/images/generations'); assert.equal(last.auth, 'Bearer agnes-test-key');
  const sent = JSON.parse(last.body);
  assert.equal(sent.image.length, 2, '人 + 场景两张图'); assert.equal(sent.model, 'agnes-image-2.5-flash');
  const opts = await j(await fetch(`${base}/image-edit/options`));
  assert.deepEqual(opts.body.saved, { provider: 'agnes', model: 'agnes-image-2.5-flash' });
  // 短剧运行带场景：场景段拼进故事
  const r = await fetch(`${base}/drama/run?story=${encodeURIComponent('告别')}&tier=local&character=${id}&scene=${encodeURIComponent(sc.body.id)}`);
  await r.text();
  const story = JSON.parse(fs.readFileSync(argsOut, 'utf-8')).find((a) => a.startsWith('story='));
  assert.match(story, /【场景已定/); assert.ok(story.includes('水箱'));
  assert.equal((await j(await fetch(`${base}/drama/run?story=x&tier=local&character=${id}&scene=nope`))).status, 404);
});
