...
const project = {id: 'demo', title: 'Demo', soundtrackSrc: 'audio/music.wav', scenes: [{narrationSrc: 'audio/voice.wav', layers: [{src: 'images/a.png'}, {src: 'https://example.com/x.png'}]}]};

test('项目打包会收集本地素材并忽略远程 URL', () => {
  assert.deepEqual(collectAssetPaths(project).sort(), ['audio/music.wav', 'audio/voice.wav', 'images/a.png']);
});
...
test('项目和素材可完整打包并安全导入', async () => {
  ...
  assert.equal(imported.assets, 3); assert.match(imported.project.scenes[0].layers[0].src, /^uploads\/imported\//);
});
