# v1 · 图层动画编辑器（Remotion 剪纸 / 信息板）

> 这是 OpenShorts v1 的路线：用图层动画（纸片剪纸、信息板）出讲书 / 技术解读 / 故事片。
> v2「开片」把它作为「图层动画」这一种画面来源保留，编辑器仍在 `/editor`。
> 只想用开片出短视频的话，看 [主 README](../README.md) 就够了；本文是 v1 编辑器、渲染脚本和项目协议的完整参考。
> 下文的命令都在仓库根目录执行。
...
完整部署说明见 [docs/deployment.md](deployment.md)。发布前可运行：
...
![三天荔枝道 六镜头](media/lychee-road-scenes.jpg)
...
![后羿射日 六镜头](media/nine-suns-scenes.jpg)
