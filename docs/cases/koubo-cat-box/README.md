> **这是早期版本，留作对照。** 当时的成片没有可见字幕、两镜纯黑、还配错过素材；
> 想看现在的效果请去 [《为什么切洋葱会流眼泪》](../koubo-onion/)——同一条流水线，
> 字幕烧进画面、画面经看图把关、素材库没货时本机现画。下面这些问题的成因都记在本页末尾。
...
>
> 3. **免 key 时的画面来源换了。** 这条片的素材全部来自 Commons 的**视频**库，而它偏科教/历史——
>    上面 s2 的高尔顿板、s4/outro 的量子分布图就是这么来的。后来发现同一个 API 换成图片检索准得多
>    （`cat cardboard box` → "Cats in cardboard boxes"），所以免 key 路径改成**以 CC 图片为主**
>    （完整图居中 + 自身放大虚化垫底 + 缓推，主体不会被裁掉），视频只留一个候选位。
>
> 重跑（`openshorts run <project.json>`）即可得到带字幕、且画面来源已换过的版本；这一页会在重跑后换成新成片。
