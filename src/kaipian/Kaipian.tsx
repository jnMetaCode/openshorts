...
export const Kaipian = () => {
  ...
  const installLocal = (what: 'sdcli' | 'model' | 'all', model = 'minimax-h3-q2') => {
    ...
    es.addEventListener('error', (e: any) => { try { setError(JSON.parse(e.data).m); } catch { setError(t('下载中断（可重试，支持断点续传）')); } es.close(); });
  };
  ...
  const installFfmpeg = () => {
    ...
    es.addEventListener('error', (e: any) => { try { setError(JSON.parse(e.data).m); } catch { setError(t('ffmpeg 下载中断，可重试')); } es.close(); });
  };
  ...
  const Aside = () => {
    ...
    return <aside className="kp-aside">
      ...
      {projects.length > 0 && <>
        <h4>{t('项目')}</h4>
        {/* 标题来自模型，同一个话题出几版就会完全重名——补上 id 和时间才分得清是哪一条 */}
        <ul className="kp-projs">{projects.slice(0, 10).map((p) => <li key={p.id} onClick={() => openProject(p.id).catch((e) => setError(e.message))} title={p.id}>
          <span>{p.final ? '🎬' : '✍️'} {p.title || p.id}</span>
          <em>{p.id}　{p.updatedAt?.slice(5, 16).replace('T', ' ')}</em>
        </li>)}</ul>
      </>}
    </aside>;
  };
  ...
  return <div className="kp">
    ...
    {step === 2 && line === 'drama' && <section className="kp-card">
      <h3>{t('出片档位')}</h3>
      <div className="kp-lines">
        <button className={tier === 'local' ? 'active' : ''} onClick={() => setTier('local')}><b>{t('本地草稿档 · 不花钱')}</b><small>{dramaOpts?.localReady ? t('本机 sd.cpp 跑 MiniMax-H3 Q2：640×384、2 秒/镜、每镜约 3–4 分钟；画质草稿级，用来验证方向') : t('未就绪：需要 sd-cli + 约 27 GB 模型（openshorts doctor 看怎么装）')}</small></button>
        <button className={tier === 'cloud' ? 'active' : ''} onClick={() => setTier('cloud')}><b>{t('云端成片档 · 按秒计费')}</b><small>{dramaOpts?.cloudProviders.length ? `${t('已配 key：')}${dramaOpts.cloudProviders.join(' / ')}` : t('还没配视频供应商 key（秘塔 / APIMart / Agnes / 火山）')}</small></button>
      </div>
      {tier !== 'cloud' && localSt && !localSt.ok && <div className="kp-keys" style={{marginTop: 0}}>
        <b>{t('本地草稿档还差：')}</b>{!localSt.cliFound && <span> sd-cli</span>}{localSt.models.filter((m) => m.usable && !m.present).length > 0 && <span>{t(' 模型文件（约 27 GB，可断点续传）')}</span>}
        <p style={{fontSize: 11, color: '#a99b8e'}}>{t('本机内存')} {localSt.memGB} GB · {t('可用档位：')}{localSt.models.filter((m) => m.usable).map((m) => m.id).join(' / ') || t('无（低于 24 GB）')} · {t('装到')} <code>{localSt.modelsDir}</code></p>
        <label className="kp-check"><input type="checkbox" checked={agree} onChange={(e) => setAgree(e.target.checked)}/> {t('我已阅读并同意 ')}{localSt.license.split('：')[0]}（<a href={localSt.license.split('：')[1]} target="_blank" rel="noreferrer">{t('条款 ↗')}</a>{t('）与 stable-diffusion.cpp 的 MIT 许可')}</label>
        <div className="kp-actions" style={{justifyContent: 'flex-start'}}>
          {!localSt.cliFound && <button disabled={!agree || !!dl?.bytes} onClick={() => installLocal('sdcli')}>{t('安装 sd-cli（预编译包）')}</button>}
          {localSt.models.filter((m) => m.usable && !m.present).slice(0, 1).map((m) => <button key={m.id} disabled={!agree || !!dl?.bytes} onClick={() => installLocal('model', m.id)}>{t('下载模型 ')}{m.id}</button>)}
        </div>
      ...
      {tier === 'cloud' && (() => {
        ...
        return <div className="kp-row">
          <label>{t('视频供应商')}<select value={cloud.video_provider} onChange={(e) => { const p = vps.find((v) => v.id === e.target.value); const m = p?.models[0]; pick({video_provider: e.target.value, video_model: m?.id ?? '', video_resolution: m?.resolutions[0] ?? '', video_duration: String(m?.durations[0] ?? '')}); }}>
            {vps.map((v) => <option key={v.id} value={v.id}>{v.id}{v.hasKey ? ' ✓' : t('（未配 key）')}</option>)}
          </select></label>
          <label>{t('视频模型')}{vp && vp.models.length ? <select value={cloud.video_model} onChange={(e) => { const m = vp.models.find((x) => x.id === e.target.value); pick({video_model: e.target.value, video_resolution: m?.resolutions[0] ?? '', video_duration: String(m?.durations[0] ?? '')}); }}>{vp.models.map((m) => <option key={m.id} value={m.id}>{m.id}</option>)}</select> : <input value={cloud.video_model} onChange={(e) => pick({video_model: e.target.value})} placeholder={t('该供应商没有已核实的模型清单，手填')}/>}</label>
          <label>{t('档位')}{vm && vm.resolutions.length ? <select value={cloud.video_resolution} onChange={(e) => pick({video_resolution: e.target.value})}>{vm.resolutions.map((r) => <option key={r}>{r}</option>)}</select> : <input value={cloud.video_resolution} onChange={(e) => pick({video_resolution: e.target.value})}/>}</label>
    ...
    {step === 2 && line === 'koubo' && <section className="kp-card">
      {ff && ff.found && !ff.subtitles && <div className="kp-warn" style={{borderColor: '#e0524b'}}>
        <b>{t('⛔ 这台机器的 ffmpeg 烧不了字幕')}</b>{t('（缺 libass）。出来的片传到抖音/视频号后')}<b>{t('看不到字')}</b>{t('；没找到素材而退成纯色底的镜头会是一块空屏。 Homebrew 现在的 ffmpeg 已经不带 libass，重装它没用。')}
        <div style={{marginTop: 8}}><button className="primary" onClick={installFfmpeg} disabled={!!dl}>{t('装一份带 libass 的（约 40 MB，只放进 ~/.openshorts/bin，不动系统 ffmpeg）')}</button></div>
      ...
      {ff && !ff.found && <div className="kp-warn" style={{borderColor: '#e0524b'}}><b>{t('⛔ 没找到 ffmpeg')}</b>{t('，出片一定失败。')}<button className="primary" onClick={installFfmpeg} disabled={!!dl} style={{marginLeft: 8}}>{t('装一份')}</button></div>}
      <h3>{t('画面来源')}</h3>
      <div className="kp-srcs">
        <SrcCard k="stock" label={t('素材库')} hint={t('Wikimedia CC 图片/视频（免 key）+ Pexels/Pixabay + 本地素材夹 · 不花钱')}/>
        <SrcCard k="image" label={t('AI 配图')} hint={t('云端文生图 · 按张计费（短剧线在用；口播线用本机出图，不花钱）')}/>
        <SrcCard k="local" label={t('本地生成')} hint={gen?.ok ? t('本机文生图就绪 · 素材库没命中时顶上 · 不花钱') : t('sd.cpp 本机文生图 · 不花钱（未装模型时素材库没命中就退纯色底）')}/>
        <SrcCard k="cloud" label={t('云端出片')} hint={t('秘塔 / 火山 / Agnes … · 按秒计费（M2）')}/>
      </div>
      <div className="kp-row">
        <label>{t('本次画面')}<select value={source} onChange={(e) => setSource(e.target.value as any)}><option value="stock">{t('素材库（找不到时用纯色底）')}</option><option value="solid">{t('只用纯色底 + 大字幕（最快，测试用）')}</option></select></label>
        <label>{t('本地素材夹（可选）')}<input value={localDir} onChange={(e) => setLocalDir(e.target.value)} placeholder={t('/path/to/我的素材（文件名或同名 .txt 里的关键词会被匹配）')}/></label>
      </div>
      {gen && gen.cliFound && !gen.ok && <details className="kp-keys">
        <summary>{t('找不到素材的镜头，让本机画一张（可选，不花钱、不联网）')}</summary>
        <p style={{fontSize: 13, margin: '6px 0'}}>
          {t('口播里"狭小空间让它感到安全"这类抽象句，素材库本来就没有对应画面，现在会退成纯色底。 装一个本地文生图模型后，这些镜头会由本机现画一张（每张几十秒）。')}
        ...
        {gen.models.every((m) => !m.usable) && <small>{t('本机内存 ')}{gen.memGB}{t(' GB，跑不动任何档位。')}</small>}
      ...
      {cfg && (!(cfg.stock.hasPexels || cfg.stock.hasPixabay) || keyTest) && <details className="kp-keys" open><summary>{t('素材库 key（免费，注册即得；只存本机 ')}<code>~/.openshorts/config.json</code>）</summary>
        <div className="kp-row">
          <label>Pexels key <a href="https://www.pexels.com/api/" target="_blank" rel="noreferrer">{t('去申请 ↗')}</a><input value={keyDraft.pexelsKey} onChange={(e) => setKeyDraft({...keyDraft, pexelsKey: e.target.value})} placeholder={t('粘贴后保存')}/></label>
          <label>Pixabay key <a href="https://pixabay.com/api/docs/" target="_blank" rel="noreferrer">{t('去申请 ↗')}</a><input value={keyDraft.pixabayKey} onChange={(e) => setKeyDraft({...keyDraft, pixabayKey: e.target.value})}/></label>
      ...
      <div className="kp-row">
        ...
        <label>{t('字幕样式')}<select value={captions} onChange={(e) => { setCaptions(e.target.value); setCapStyle({}); }}><option value="douyin">{t('抖音黄字描边')}</option><option value="clean">{t('简约白')}</option><option value="boxed">{t('黑底白字')}</option></select></label>
      </div>
      <details className="kp-keys"><summary>{t('字幕细调（不改就用预设）')}</summary>
        <div className="kp-row">
          ...
          <label>{t('位置')}<select value={capStyle.position ?? 'bottom'} onChange={(e) => setCapStyle({...capStyle, position: e.target.value})}><option value="bottom">{t('底部')}</option><option value="middle">{t('中部')}</option><option value="top">{t('顶部')}</option></select></label>
      ...
      <div className="kp-cost"><b>{t('本次花费：0 元')}</b><small>{t('素材库 / 本地素材 / 纯色底不花钱 · Edge TTS 免费 · 合成用本机 ffmpeg')}{sources && !sources.tools.ffmpeg ? t(' ⛔ 未检测到 ffmpeg，出片会失败') : ''}</small></div>
    ...
    {step === 3 && line === 'drama' && !project && <section className="kp-card">
      ...
      <div className="kp-warn">{t('云端任务一旦创建就会计费，中途关页面不会取消服务商那边的任务；本地出片关页面即停。')}</div>
    </section>}
    ...
    {step === 3 && project && <section className="kp-card">
      ...
      <div className="kp-actions">
        ...
        <button className="primary" disabled={!!busy} onClick={() => runProject()}>{busy ? busy : t('出片 →')}</button>
      </div>
    </section>}
    ...
    {step === 4 && project?.line === 'drama' && <section className="kp-card kp-final">
      ...
      <div className="kp-drama-shots">{(project.shots as unknown as DramaShot[]).map((s) => <div key={s.id} className="kp-drama-shot">
        ...
        {redo?.shot === s.id && <div className="kp-redo">
          <textarea rows={2} value={redo.feedback} onChange={(e) => setRedo({...redo, feedback: e.target.value})} placeholder={t('想怎么改（可空：只换来源重出）')}/>
          {s.kind === 'video' && <select value={redo.tier} onChange={(e) => setRedo({...redo, tier: e.target.value as any})}><option value="same">{t('同一来源')}</option><option value="local" disabled={!dramaOpts?.localReady}>{t('本地草稿档（不花钱）')}</option><option value="cloud">{t('云端成片档（按秒计费，用第 2 步填的供应商）')}</option></select>}
          <button className="primary" disabled={!!busy} onClick={() => dramaRedo(s.id, redo.feedback, redo.tier)}>{t('重出这一镜')}</button>
          <small>{t('上游剧本 / 定妆图 / 其他镜头原样复用；合成会自动重跑。')}</small>
        </div>}
      </div>)}</div>
      ...
      <div className="kp-warn">{t('草稿满意后切「云端成片档」用同一段故事重出；单镜重出用上面每镜的按钮（只重跑该镜及下游，上游剧本与定妆图不再花钱）。')}</div>
    </section>}
    ...
    {step === 4 && project?.line !== 'drama' && project?.final && <section className="kp-card kp-final">
      <div className="kp-final-grid">
        ...
        <div>
          ...
          <h4>{t('发布包')}</h4>
          <div className="kp-inline"><select value={platform} onChange={(e) => setPlatform(e.target.value)}><option value="douyin">{t('抖音')}</option><option value="shipinhao">{t('视频号')}</option><option value="bilibili">{t('B 站')}</option><option value="shorts">YouTube Shorts</option></select><button onClick={makePack} disabled={!!busy}>{t('打发布包（mp4 + 封面 + SRT + 文案）')}</button></div>
          {pack && <p style={{fontSize: 12}}>{t('已生成：')}<code>{pack.dir}</code>{pack.zipName ? ` · ${pack.zipName}` : ''}<br/><small>{t('不自动发布——拖进平台后台即可；AI 标识与素材署名都在文案里。')}</small>
          ...
          <div className="kp-row">
            ...
            <label>{t('字幕样式（多选）')}<select multiple size={3} value={batchCaptions} onChange={(e) => setBatchCaptions([...e.target.selectedOptions].map((o) => o.value))}><option value="douyin">{t('抖音黄字描边')}</option><option value="clean">{t('简约白')}</option><option value="boxed">{t('黑底白字')}</option></select></label>
          </div>
          <div className="kp-actions" style={{justifyContent: 'flex-start'}}><button onClick={runBatch} disabled={!!busy || (batchVoices.length || 1) * (batchCaptions.length || 1) > 12}>{lang === 'en' ? `Render ${(batchVoices.length || 1) * (batchCaptions.length || 1)} versions` : `出 ${(batchVoices.length || 1) * (batchCaptions.length || 1)} 版`}</button></div>
          {batchResults.length > 0 && <ul className="kp-copy">{batchResults.map((r) => <li key={r.id} style={{cursor: 'default'}}>{r.ok ? '✅' : '⛔'} {r.id}{r.ok ? <> · {r.durationSec?.toFixed(1)}s · <a href={`/api/kaipian/projects/${encodeURIComponent(project.id)}/variant/${encodeURIComponent(r.id)}`} download>{t('下载')}</a></> : ` ${r.error}`}</li>)}</ul>}
