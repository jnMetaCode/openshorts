...
const provider=String(args.provider??'auto');const voice=String(args.voice??'zh-CN-XiaoxiaoNeural');const timings=[];let cursor=0;
...
const macos=async(text,wav)=>{if(process.platform!=='darwin')throw new Error('非 macOS');const cache=path.resolve('data/swift-cache');await fs.mkdir(cache,{recursive:true});await run('swift',['scripts/macos-tts.swift',text,wav,String(args.rate??0.48)],{env:{...process.env,SWIFT_MODULECACHE_PATH:cache,CLANG_MODULE_CACHE_PATH:cache}});};
const edge=async(text,wav)=>{const mp3=`${wav}.mp3`;await run('edge-tts',['--voice',voice,'--text',text,'--write-media',mp3]);await run('ffmpeg',['-y','-v','error','-i',mp3,'-ar','48000','-ac','1',wav]);await fs.rm(mp3,{force:true});};
for(const [index,segment] of story.segments.entries()){
  const basename=`${String(index+1).padStart(2,'0')}-${segment.id}`;const wav=path.join(output,`${basename}.wav`);await fs.rm(wav,{force:true});let used='';const errors=[];
  for(const candidate of provider==='auto'?['macos','edge']:[provider]){try{await (candidate==='macos'?macos:edge)(segment.text,wav);if(await durationOf(wav)>.2){used=candidate;break;}throw new Error('生成了空音频');}catch(error){errors.push(`${candidate}: ${error.message}`);await fs.rm(wav,{force:true});}}
  const duration=await durationOf(wav);if(!used||duration<=.2)throw new Error(`旁白 ${basename} 生成失败。不会输出静默成片。\n${errors.join('\n')}\n可在普通终端运行 npm run story:audio，或安装 edge-tts 后使用 --provider=edge。`);
...
await fs.writeFile(path.join(output,'timings.json'),`${JSON.stringify({storyId:story.id,voice,totalDuration:cursor,segments:timings},null,2)}\n`);
