...
const provider=String(args.provider??'auto');const voice=String(args.voice??'zh-CN-YunjianNeural');const edgeRate=String(args.rate??'+2%');const edgePitch=String(args.pitch??'-5Hz');const timings=[];let cursor=0;const cacheDir=path.resolve('data/tts-cache',story.id);await fs.mkdir(cacheDir,{recursive:true});
