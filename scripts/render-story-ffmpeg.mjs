...
const total=absoluteFrame/project.fps;filters.push(`${mix.join('')}amix=inputs=${mix.length}:duration=longest:normalize=0,atrim=0:${total.toFixed(3)},loudnorm=I=-18:LRA=9:TP=-1.5[a]`);const output=path.join(root,'out',`${project.id}.mp4`);await fs.mkdir(path.dirname(output),{recursive:true});ffargs.push('-filter_complex',filters.join(';'),'-map','0:v','-map','[a]','-c:v','copy','-c:a','aac','-b:a','192k','-t',total.toFixed(3),'-movflags','+faststart',output);await run('ffmpeg',ffargs,{maxBuffer:20*1024*1024});
